...
const app = createApp({ db, env: { CORS_ORIGIN: "http://localhost:4321", RATE_LIMIT_PER_MINUTE: "1000" } });
...
for (let index = 0; index < 100; index += 1) {
  ...
  const response = await app.request(`http://localhost/api/search?q=${encodeURIComponent(query)}&limit=20`);
...
console.log(`API_BENCHMARK ${JSON.stringify({
